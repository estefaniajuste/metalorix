...
export function RangeSelector({ active, onChange }: RangeSelectorProps) {
  ...
  return (
    <div
      className="overflow-x-auto max-w-full scrollbar-hide -mx-1 px-1"
      role="tablist"
      aria-label={t("timeRange")}
    >
      <div className="flex items-center gap-0.5 bg-surface-2 rounded-sm p-1 w-max">
        {ranges.map((range) => (
          <button
            key={range}
            role="tab"
            ...
            className={`
              px-2.5 sm:px-3.5 py-1.5 rounded-xs text-[13px] font-semibold transition-all duration-250 ease-smooth whitespace-nowrap
              ${
                active === range
